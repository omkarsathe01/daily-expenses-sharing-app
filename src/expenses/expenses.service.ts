...
export class ExpensesService {
  ...
  async create(createExpenseDto: CreateExpenseDto): Promise<ExpenseResponseDto> {
    const { amount, description, paid_to, paid_by, shares, split_type } = createExpenseDto;
    ...
    if (paid_to[0]?.email) {
      let receiverUser = await this.usersService.findOneByEmail(paid_to[0].email);
      if (receiverUser) {
        receiverUser.balance += amount;
      } else {
        receiverUser = await this.usersService.createUserByEmail({ email: paid_to[0].email });
        receiverUser.balance += amount;
      }
  ...
  private async handleEqualSplit(shares: UserShareDto[], totalAmount: number, paidBy: string, expense: Expense) {
  ...
  private async handleExactSplit(shares: UserShareDto[], totalAmount: number, paidBy: string, expense: Expense) {
    const calculatedAmount = shares.reduce((sum, share) => sum + (share.amount || 0), 0);

    if (calculatedAmount !== totalAmount) {
      throw new BadRequestException(`Total shared amount (${calculatedAmount}) does not match the expense amount (${totalAmount}).`);
    }
  
    for (const share of shares) {
      if (share.amount === undefined) {
        throw new BadRequestException('Please specify the exact amount for each share.');
      }
  ...
  private async handlePercentSplit(shares: UserShareDto[], totalAmount: number, paidBy: string, expense: Expense) {
    const totalPercentage = shares.reduce((sum, share) => sum + (share.percent || 0), 0);

    if (totalPercentage !== 100) {
      throw new BadRequestException('Total percentage does not add up to 100%.');
    }
  ...
  private async updateUserShare(share: UserShareDto, paidBy: string, amount: number, expense: Expense) {
    ...
    expense.shares?.push({ email: share.email, amount, percent: share.percent || null });
